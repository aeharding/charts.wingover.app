"""Populated places that must always have chart, per region.

Its own module with NO heavy imports: coverage-check.py runs inside the
GDAL container, but the verify job runs on a plain runner and needs the
same list to check shipped tiles. The two gates must never disagree
about what "has chart" means.
"""
